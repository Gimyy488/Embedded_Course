...
int main(void){
	...
	DIO_enuSetPinDirection(DIO_GROUP_B,DIO_PIN_0,DIO_OUTPUT);
	struct strFuncParameter Local_strFuncParameter = {false,0};
	TIMER_enuInit();
	Lcd_enuInit();
	Led_enuInit();
	Lcd_enuSendCommand(LCD_CURSOR_OFF);
	Lcd_enuSendString("LED : ");
	TIMER_enuSetCallBack(vidISRFunc,&Local_strFuncParameter);
	GIE_vidEnable();
	TIMER_enuSetOCRValue(0,9);
...
int main(void){
	...
	TIMER_enuSetCallBack(vidISRFunc,&Local_strFuncParameter);
	GIE_vidEnable();
	TIMER_enuAsyncDelayms(TIMER_INT0,2000);
	Led_enuLedToggle(LED_GREEN_ID);
...
/*int main(void){
	...
	while(1){
		...
		TIMER_enuSyncDelayms(TIMER_INT0,4000);
		Led_enuLedToggle(LED_GREEN_ID);
		TIMER_enuSyncDelayms(TIMER_INT0,4000);
		Led_enuLedToggle(LED_GREEN_ID);
	}
